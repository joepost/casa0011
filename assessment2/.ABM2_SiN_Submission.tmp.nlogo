...
to check-collision  ;; cars check whether a cyclist is on the same patch (criteria for a collision)
  ...
  ask cars [
    if any? cyclists-here [
      ...
      [  ;; IF LEARNING IS ON:
        set potential-collisions count cyclists-here             ; only count potential collisions with cyclists, not other motorists
        ifelse random-float 1 < (1 - association)
        [                                                        ; collisions = potential collisions by probability of car being prepared for cyclist (1 - association)
          set collisions potential-collisions                    ;  a higher association results in lower likelihood of potential collision becoming actual collision
        ]
        [
          set collisions 0
        ]
