; CASA0011 Week 5
; Fireflies Model: Testing the Behaviour Space
;
; Date: 2023-02-06
...
globals
[
  synchronised?    ;boolean variable to mark if fireflies have synchronised
  synch_time       ;time at which fireflies synchronised
]
...
to monitor
  ...
  let flash_number count (turtles with [color = yellow])   ;temporary variables to count the number of fireflies flashing
  if flash_number = number [               ;check if all fireflies are flashing together - if so, record the synch time
    set synchronised? true
    set synch_time ticks
  ]
...
@#$#@#$#@
<experiments>
  <experiment name="Time-to-synchronisation" repetitions="10" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="10000"/>
    <exitCondition>synchronised? = true</exitCondition>
    <metric>synch_time</metric>
    ...
    <steppedValueSet variable="cycle-length" first="5" step="10" last="55"/>
